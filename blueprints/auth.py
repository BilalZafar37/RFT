...
class User(UserMixin):
    def __init__(self, user_id, username, role, department, ip, email, Fullname):
        self.id = user_id
...
def load_user(user_id):
    ...
    user = model.query(Users).filter_by(UserID=int(user_id)).first()
    model.close()
    print(f"User found: {user.UserID}")
    if user:
        return User(user.UserID, user.Username, user.Role, user.Department, user.IP, user.Email, user.Fullname)
    return None
...
def login():
    ...
    if request.method == "POST":
        ...
        if user_data:
            ...
            # user_report_access = model.query(UserReports).filter_by(UserID = current_user.id).all()
            # user_report_access = [u.report.ReportName for u in user_report_access]

            session["user_site_access"] = user_site_access
            session["user_brand_access"] = user_brand_access
            # session['user_report_access'] = user_report_access
